...
import (
	...
	internalsys "github.com/taubyte/vm-wasm-utils/sys"
...
func (m *CallContext) ExportedMemoryDefinitions() map[string]api.MemoryDefinition {
	// TODO: Not necessary at the moment, but need to implement
	return nil
}

func (m *CallContext) ExportedFunctionDefinitions() map[string]api.FunctionDefinition {
	definitions := make(map[string]api.FunctionDefinition, 0)
	for _, v := range m.module.Functions {
		definitions[v.definition.Name()] = v.definition
	}
	return definitions
}
